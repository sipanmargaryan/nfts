...
def create_new_company(db: Session, user_id: int, company: CompanyCreateSchema) -> CompanyProfile:
...
def user_owns_company(db: Session, user_id: int, company_id: int) -> CompanyProfile | None:
    company = (
        db.query(CompanyProfile)
        .filter(
            CompanyProfile.account_id == user_id,
