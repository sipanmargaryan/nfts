...
def get_active_user_by_id( db: Session, user_id: int) -> Type[Account] | None:
    return db.query(Account).filter_by(id=user_id, active=True).first()

def get_active_user_by_email(db: Session, email: str) -> Type[Account] | None:
...
def check_user_reset_password_code(db: Session, code: str) -> Type[Account] | None:
